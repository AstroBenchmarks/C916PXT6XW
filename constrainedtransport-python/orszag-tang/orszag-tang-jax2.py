...
def hydro_mhd2d_fluxes(rho, vx, vy, P, bx, by, gamma, dx, dt):
    ...
    Bx, By = getBavg(bx, by)
    Mass, Momx, Momy, Energy = getConserved(rho, vx, vy, P, Bx, By, gamma, dx**2)
    ...
    Bx, By = getBavg(bx, by)
    rho, vx, vy, P = getPrimitive(Mass, Momx, Momy, Energy, Bx, By, gamma, dx**2)
...
def main():
    ...
    outputCount = 1
    num_timesteps = 100 * int(N / 32)  # XXX
    dt = tEnd / num_timesteps

    # Simulation Main Loop
    start_time = time.time()
    for i in range(num_timesteps):
        rho, vx, vy, P, bx, by = hydro_mhd2d_fluxes(
            rho, vx, vy, P, bx, by, gamma, dx, dt
        )
        t += dt
